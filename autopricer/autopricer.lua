require "List"
require "gui"  -- This already imports the UpdateGUI function
require('chat')
...
function get_sales(item, stack)
    ...
    local history = {}
    local header = {}
    local title, stock, server, rating, true_rating, last_saleon
    
    header['cookie'] = temp_header or servers[settings.server] or servers[defaults.server]
    local result_table = {}
    local result, err = https.request{
    ...
    if err then
        windower.add_to_chat(167, "Error fetching data: " .. err)
        return
    end
    ...
    if not t then
        windower.add_to_chat(167, "Could not parse sales data from response")
        return
    end
...
function get_sales_auto(item, stack)
    ...
    for i = 0, price_list:size()-1 do
        local cached = price_list:at(i)
        if cached.id == item then
    ...
    local history = {}
    local header = {}
    local title, stock, server, rating, true_rating, last_saleon, median
    ...
    local ability = ItemPrice.new(item, info)
    price_list:push_back(ability)
    
    -- Call the UpdateGUI function from gui.lua
